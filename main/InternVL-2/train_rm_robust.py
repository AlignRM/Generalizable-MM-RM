"""An example of implementing Shortcut-aware InternVL2 Reward Modeling (RM)."""
...
from transformers import GPTQConfig, deepspeed, PreTrainedModel
...
from models.internvl_chat.modeling_internvl_reward_robust import InternVLRewardModel
...
class InternRewardDataCollator(RewardDataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        features_chosen_fake = []
        features_rejected_fake = []
            ...
            features_common.append(
                {
                    ...
                    "pixel_values_fake": feature['pixel_values_fake'],
                    "image_flags_fake": feature['image_flags_fake'],
                }
            )
            features_chosen_fake.append(
                {
                    "input_ids": feature["input_ids_chosen_fake"],
                    "attention_mask": feature["attention_mask_chosen_fake"],
                }
            )
            features_rejected_fake.append(
                {
                    "input_ids": feature["input_ids_rejected_fake"],
                    "attention_mask": feature["attention_mask_rejected_fake"],
                }
            )
            if has_margin:
        ...
        batch_chosen_fake = self.tokenizer.pad(
            features_chosen_fake,
            padding=self.padding,
            max_length=self.max_length,
            pad_to_multiple_of=self.pad_to_multiple_of,
        ...
        batch_rejected_fake = self.tokenizer.pad(
            features_rejected_fake,
            padding=self.padding,
            max_length=self.max_length,
            pad_to_multiple_of=self.pad_to_multiple_of,
        ...
        batch = {
            ...
            "return_loss": True,
                # below items are used by the text-only branch
                "input_ids_chosen_fake": batch_chosen_fake['input_ids'],
                "attention_mask_chosen_fake": batch_chosen_fake["attention_mask"],
                "input_ids_rejected_fake": batch_rejected_fake['input_ids'],
                "attention_mask_rejected_fake": batch_rejected_fake["attention_mask"],
                "pixel_values_fake": torch.concat([f['pixel_values_fake'] for f in features_common]),
                "image_flags_fake": torch.concat([f['image_flags_fake'] for f in features_common]),   
        }
        if has_margin:
...
def compute_loss(
    ...
    chosen_return = model(
        ...
        return_dict=True,
        input_ids_fake = inputs['input_ids_chosen_fake'],
        attention_mask_fake = inputs['attention_mask_chosen_fake'],
        pixel_values_fake = inputs['pixel_values_fake'],
        image_flags_fake = inputs['image_flags_fake'],)
    rewards_chosen_main = chosen_return[0]['logits']
    rewards_chosen_bias = chosen_return[1]['logits']

    rejected_return = model(
        input_ids=inputs["input_ids_rejected"],
        attention_mask=inputs["attention_mask_rejected"],
        pixel_values=inputs['pixel_values'],
        image_flags=inputs['image_flags'],
        return_dict=True,
        input_ids_fake = inputs['input_ids_rejected_fake'],
        attention_mask_fake = inputs['attention_mask_rejected_fake'],
        pixel_values_fake = inputs['pixel_values_fake'],
        image_flags_fake = inputs['image_flags_fake'],)
    rewards_rejected_main = rejected_return[0]['logits']
    rewards_rejected_bias = rejected_return[1]['logits']

    bias_logit_diff = rewards_chosen_bias - rewards_rejected_bias
    bias_logit_diff_detached = bias_logit_diff.detach()
    main_logit_diff = rewards_chosen_main - rewards_rejected_main
    main_logit_diff_detached = main_logit_diff.detach()

    assert 'margin' not in inputs
    epison = 1e-7

    scale_1 = (epison / 2 - nn.functional.logsigmoid(bias_logit_diff_detached) / (epison - nn.functional.logsigmoid(bias_logit_diff_detached) - nn.functional.logsigmoid(main_logit_diff_detached)))
    loss = torch.mean(-nn.functional.logsigmoid(main_logit_diff) * scale_1 - nn.functional.logsigmoid(bias_logit_diff))

    if return_outputs:
        return loss, {
            "rewards_chosen_main": rewards_chosen_main,
            "rewards_rejected_main": rewards_rejected_main,
            "rewards_chosen_bias": rewards_chosen_bias,
            "rewards_rejected_bias": rewards_rejected_bias
        }
    return loss
...
class LazyInternRewardDataset(Dataset):
    ...
    def get_item(
        ...
        # use get_item_delta for our shortcut-aware method
    ...
    def get_item_delta(
        ...
        assert not self.use_image_only and not self.use_text_only
        ...
        # load image
        image_fake = Image.new('RGB', (224, 224), (255, 255, 255))
        image_real = Image.open(img_path).convert('RGB')
        if self.dynamic_image_size:  # If dynamic image size is enabled, preprocess the image dynamically
            images_real = dynamic_process_image(image_real, min_num=self.min_dynamic_patch, max_num=self.max_dynamic_patch,
                                        image_size=self.image_size, use_thumbnail=self.use_thumbnail)
        else:  # Otherwise, use the original image as a single patch
            images_real = [image_real]
        images_fake = [image_fake]

        # Apply the transformation to each image and stack the results into a tensor
        pixel_values_real = [self.transform(image) for image in images_real]
        pixel_values_real = torch.stack(pixel_values_real)

        pixel_values_fake = [self.transform(image) for image in images_fake]
        pixel_values_fake = torch.stack(pixel_values_fake)

        # Ensure that there is only one patch if dynamic image size is not enabled
        num_patches_real = pixel_values_real.size(0)
        num_patches_fake = 1

        # Select the appropriate preprocessing function based on the template name
        preprocess_function = self.get_preprocess_function()
        chosen_ret_real = preprocess_function(self.template_name, [deepcopy(chosen_conv)],
                                  self.tokenizer, [self.num_image_token * num_patches_real], truncation = True)
        rejected_ret_real = preprocess_function(self.template_name, [deepcopy(rejected_conv)],
                                  self.tokenizer, [self.num_image_token * num_patches_real], truncation = True)

        chosen_ret_fake = preprocess_function(self.template_name, [deepcopy(chosen_conv)],
                                  self.tokenizer, [self.num_image_token * num_patches_fake], truncation = True)
        rejected_ret_fake = preprocess_function(self.template_name, [deepcopy(rejected_conv)],
                                  self.tokenizer, [self.num_image_token * num_patches_fake], truncation = True)


        return {
                "input_ids_chosen": chosen_ret_real['input_ids'][0],
                "attention_mask_chosen": chosen_ret_real["attention_mask"][0],
                "input_ids_rejected": rejected_ret_real['input_ids'][0],
                "attention_mask_rejected": rejected_ret_real["attention_mask"][0],
                "pixel_values": pixel_values_real,
                "image_flags": torch.tensor([1] * num_patches_real, dtype=torch.long),
                # below items are used by the text-only branch
                "input_ids_chosen_fake": chosen_ret_fake['input_ids'][0],
                "attention_mask_chosen_fake": chosen_ret_fake["attention_mask"][0],
                "input_ids_rejected_fake": rejected_ret_fake['input_ids'][0],
                "attention_mask_rejected_fake": rejected_ret_fake["attention_mask"][0],
                "pixel_values_fake": pixel_values_fake,
                "image_flags_fake": torch.tensor([1] * num_patches_fake, dtype=torch.long),               
            }


    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        data_item = self.dataset[i]
        ret = self.get_item_delta(data_item)
        return ret



def wrap_model_tokenizer_config(model_args, training_args, data_args, lora_args):
    bias_model_path = model_args.bias_model_path  # the text-only branch, usually the same as the main branch path

    print(model_args.model_name_or_path)
    def wrap_config(model_name_or_path : str):
    ...
    config = wrap_config(model_args.model_name_or_path)
    bias_config = wrap_config(bias_model_path)
    ...
    bias_model = InternVLRewardModel.from_pretrained(
        bias_model_path,
        config=bias_config,
        cache_dir=training_args.cache_dir,
    ...
    kwargs = {'bias_reward_model': bias_model}
    main_model = InternVLRewardModel.from_pretrained(
        model_args.model_name_or_path,
        config=config,
        cache_dir=training_args.cache_dir,
        torch_dtype=torch.bfloat16,
    ...
    wrap_model(bias_model)
    wrap_model(main_model)

    return main_model, tokenizer, config
